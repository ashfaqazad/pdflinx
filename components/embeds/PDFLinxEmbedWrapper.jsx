...
export default function PDFLinxEmbedWrapper({ tool = "compress-pdf", compact = false }) {
  ...
  // ✅ BASE: localhost test ke liye .env.local mein set karo:
  // NEXT_PUBLIC_PDFLINX_URL=http://localhost:3001
  // Live pe yeh variable nahi hoga → pdflinx.com use hoga automatically
  const BASE = process.env.NEXT_PUBLIC_PDFLINX_URL || "https://pdflinx.com";

  // ✅ ?ref=embed — embed traffic tracking
  const embedSrc = compact
    ? `${BASE}/embed/${tool}?compact=true&ref=embed`
    : `${BASE}/embed/${tool}?ref=embed`;

  const fullUrl = `${BASE}/${tool}`;
