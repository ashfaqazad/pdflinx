...
/**
 * Headless upload component — sirf file input + drag logic.
 * UI completely controlled by parent (UploadLandingStep).
 *
 * Props:
 *  onFilesSelect  — callback(files[])
 *  accept         — MIME string
 *  multiple       — boolean
 *  children       — render prop: ({ open, dragging }) => JSX
 *
 * Legacy props (uploadTitle, uploadSubtitle, uploadInfo, buttonClassName, buttonLabel)
 * are IGNORED — parent handles all UI.
 */
export default function UploadStep({
  ...
  return (
    <>
      ...
      {/* Render prop pattern — parent draws all UI */}
      {typeof children === "function" ? (
        children({ open, dragging })
      ) : (
        /* Fallback: invisible drag overlay so parent's box still works */
        <div
          onDrop={handleDrop}
...
//       }}
//       onDragLeave={() => setDragging(false)}
//       className={`mx-auto flex min-h-[320px] w-full max-w-[520px] cursor-pointer flex-col items-center justify-center rounded-[20px] border-2 border-dashed px-8 py-10 text-center transition-all ${
//         dragging
//           ? "border-orange-500 bg-orange-50"
//           : "border-slate-300 bg-white hover:border-orange-500 hover:shadow-[0_0_0_4px_rgba(232,66,10,0.08)]"
//       }`}
//     >
//       <input
//         ref={inputRef}
...
//         }}
//         className="mt-6 inline-flex items-center gap-2 rounded-full bg-orange-600 px-8 py-3 text-sm font-bold text-white shadow-md transition hover:bg-orange-700 active:scale-95"
//       >
...
//       {uploadInfo && (
//         <div className="mt-5 max-w-[330px] text-center text-xs font-medium leading-5 text-slate-500">
//           {uploadInfo}
//         </div>
//       )}
//     </div>
