...
//       {activeTab === "range" && (
//         <div className="space-y-4">
//           <p className="text-xs font-semibold uppercase tracking-wide text-slate-500">Range mode:</p>

//           <div className="flex gap-2">
//             {["custom", "fixed", "smart"].map((mode) => (
//               <button
//                 key={mode}
//                 type="button"
//                 onClick={() => setRangeMode(mode)}
//                 className={`flex-1 rounded-xl border px-2 py-2 text-xs font-semibold capitalize transition
//                   ${rangeMode === mode
//                     ? "border-[#f24d0d] bg-white text-[#f24d0d] shadow-sm"
...
//                     <p className="text-xs font-bold text-slate-600">Range {index + 1}</p>
//                     {ranges.length > 1 && (
//                       <button
//                         type="button"
...
//                       <span className="text-sm text-slate-500">from page</span>
//                       <input
//                         type="number"
//                         min={1}
//                         max={totalPages}
...
//                         onChange={(e) =>
//                           setRanges((prev) =>
//                             prev.map((r, i) => i === index ? { ...r, from: Number(e.target.value) } : r)
//                           )
//                         }
//                         className="w-16 text-right text-sm font-bold text-slate-800 outline-none"
//                       />
//                     </div>
//                     <div className="flex items-center justify-between rounded-xl border border-slate-200 bg-white px-3 py-2.5">
...
//                         onChange={(e) =>
//                           setRanges((prev) =>
//                             prev.map((r, i) => i === index ? { ...r, to: Number(e.target.value) } : r)
//                           )
//                         }
//                         className="w-16 text-right text-sm font-bold text-slate-800 outline-none"
//                       />
...
//                   className="flex items-center gap-1.5 rounded-xl border border-[#f24d0d] px-4 py-2 text-sm font-semibold text-[#f24d0d] transition hover:bg-red-50"
//                 >
//                   <span className="text-base leading-none">+</span> Add Range
//                 </button>
//               </div>
//             </div>
//           )}
...
//               <p className="text-sm font-semibold text-slate-700">Fixed ranges</p>
//               <p className="text-xs leading-5 text-slate-500">Automatically split PDF after every fixed number of pages.</p>
//               <input
//                 type="number"
//                 min={1}
//                 placeholder="Pages per split"
//                 className="w-full rounded-xl border border-slate-200 bg-white px-3 py-3 text-sm font-semibold text-slate-700 outline-none focus:border-[#f24d0d]"
//               />
//             </div>
//           )}
...
//               <p className="text-sm font-semibold text-blue-800">Smart split</p>
//               <p className="mt-1 text-xs leading-5 text-blue-700">AI-based smart splitting coming soon.</p>
//             </div>
//           )}
//         </div>
//       )}
...
// // import { useState, useEffect, useRef, useCallback } from "react";
// // import { Scissors, GitMerge, FileText, Minimize2, Lock, X, CheckCircle } from "lucide-react";
// // import Script from "next/script";
// // import { useProgressBar } from "@/hooks/useProgressBar";
// // import { useToolFlow } from "@/hooks/useToolFlow";
// // import ToolPageLayout from "@/components/ToolFlow/ToolPageLayout";
...
// // // ── Split Options Sidebar ───────────────────
// // function SplitSidebarOptions({ activeTab, onTabChange, extractMode, onExtractModeChange, selectedPages, totalPages, onConvert, files,   // ADD
// //   rangeMode,
// //   setRangeMode,
// //   ranges,
// //   setRanges,
...
// //   const outputCount = extractMode === "all"
// //     ? totalPages
...
// //       {activeTab === "pages" && (
// //         <div className="space-y-3">
// //           <p className="text-xs font-semibold text-slate-500 uppercase tracking-wide">Extract mode:</p>
...
// //               ${extractMode === "all"
// //                 ? "border-[#f24d0d] bg-white text-[#f24d0d] shadow-sm ring-1 ring-[#f24d0d]"
...
// //                             );
// //                           }}
// //                           className="w-full rounded-xl border border-slate-200 bg-white px-3 py-2.5 text-sm font-semibold text-slate-700 outline-none transition focus:border-[#f24d0d]"
// //                         />
// //                       </div>
...
// //                   ]);
// //                 }}
// //                 className="w-full rounded-xl border border-dashed border-slate-300 bg-slate-50 px-4 py-3 text-sm font-semibold text-slate-500 transition hover:border-[#f24d0d] hover:bg-white hover:text-[#f24d0d]"
// //               >
// //                 + Add another range
// //               </button>
...
// //                 Automatically split PDF after every fixed number of pages.
// //               </p>
...
// //                 min={1}
// //                 placeholder="Pages per split"
// //                 className="mt-4 w-full rounded-xl border border-slate-200 bg-white px-3 py-3 text-sm font-semibold text-slate-700 outline-none focus:border-[#f24d0d]"
// //               />
// //             </div>
// //           )}
...
// //                 AI-based smart splitting coming soon.
// //                 Detect sections automatically and split intelligently.
// //               </p>
// //             </div>
// //           )}
...
// //       {/* Range tab — coming soon placeholder */}
// //       {/* {activeTab === "range" && (
// //         <div className="rounded-xl border border-slate-200 bg-slate-50 p-4 text-center text-sm text-slate-400">
// //           <p className="font-semibold text-slate-500">Split by Range</p>
// //           <p className="mt-1 text-xs">Define custom page ranges to split into separate PDFs.</p>
// //           <p className="mt-2 text-xs text-slate-400 italic">Coming soon</p>
// //         </div>
// //       )} */}

// //       {/* Size tab — coming soon placeholder */}
// //       {/* {activeTab === "size" && (
// //         <div className="rounded-xl border border-slate-200 bg-slate-50 p-4 text-center text-sm text-slate-400">
// //           <p className="font-semibold text-slate-500">Split by File Size</p>
// //           <p className="mt-1 text-xs">Automatically split PDF into chunks of a defined file size.</p>
// //           <p className="mt-2 text-xs text-slate-400 italic">Coming soon</p>
// //         </div>
// //       )} */}
...
// //         type="button"
// //         onClick={onConvert}
// //         disabled={!files.length || (extractMode === "select" && selectedPages.length === 0)}
// //         className={`w-full rounded-xl px-5 py-4 text-base font-bold text-white transition active:scale-[0.98]
// //           ${files.length && (extractMode === "all" || selectedPages.length > 0)
...
// // // ── Custom Options Full-Width Layout ───────
// // function SplitOptionsLayout({ file, onRemoveFile, onConvert, selectedPages, setSelectedPages, extractMode, setExtractMode, activeTab, setActiveTab, totalPages, setTotalPages,
// //   // ADD
// //   rangeMode,
// //   setRangeMode,
// //   ranges,
...
// //             </div>
// //             <div>
// //               <p className="text-sm font-bold text-slate-800 truncate max-w-[280px]">{file?.name}</p>
// //               <p className="text-xs text-slate-400">
// //                 {totalPages > 0 ? `${totalPages} page${totalPages !== 1 ? "s" : ""}` : "Loading..."}
// //               </p>
// //             </div>
...
// //             type="button"
// //             onClick={() => onRemoveFile(0)}
// //             className="flex h-8 w-8 items-center justify-center rounded-full bg-white text-slate-300 shadow-sm hover:bg-red-50 hover:text-red-500 transition"
// //           >
// //             <X className="h-4 w-4" />
// //           </button>
// //         </div>
...
// //     // Agar select mode hai toh selected pages bhi bhejo
// //     if (extractMode === "select" && selectedPages.length > 0) {
// //       formData.append("pages", selectedPages.sort((a, b) => a - b).join(","));
// //     }
...
// //   // customOptionsLayout — sirf OPTIONS step pe render hoga
// //   const customOptionsLayout = flow.files.length > 0 ? (
// //     <SplitOptionsLayout
// //       file={flow.files[0]}
// //       onRemoveFile={handleRemoveFile}
...
// //         // ✅ customOptionsLayout — iLovePDF style full layout
// //         customOptionsLayout={customOptionsLayout}

// //         // Options (fallback — customOptionsLayout override kar deta hai)
// //         optionsTitle="Split options"
// //         showOutputFormat={false}
// //         showPreserveLayout={false}
// //         optionSectionLabel=""
...
// //                 desc: "Select your PDF file — any size, any number of pages. Drag and drop supported on all devices.",
// //               },
// //               {
// //                 n: "2",
// //                 title: "Click Split PDF",
// //                 desc: "Hit the Split button — the tool separates every page into its own individual PDF file automatically.",
// //               },
// //               {
// //                 n: "3",
// //                 title: "Download ZIP",
// //                 desc: "All split pages are packaged into a ZIP file — download and extract the individual PDF pages you need.",
// //               },
// //             ],
// //             whyTitle: "Why Use PDFLinx Split PDF Tool?",
// //             faqTitle: "Split PDF FAQs",
...
// //                 a: "Yes. PDFLinx Split PDF is completely free — no hidden charges, no subscription required.",
// //               },
// //               {
...
// //                 a: "Every page in your PDF is separated into its own individual PDF file. All split pages are packaged into a ZIP file for download.",
// //               },
// //               {
// //                 q: "Will the quality of my PDF pages change after splitting?",
// //                 a: "No. PDFLinx extracts the original page data directly — no re-rendering or compression. Text and images stay exactly as they were.",
// //               },
// //               {
// //                 q: "Can I extract only specific pages from a PDF?",
// //                 a: "Split the PDF to get all pages as individual files, then keep only the specific page PDFs you need from the ZIP.",
// //               },
// //               {
// //                 q: "Are my uploaded PDF files safe and private?",
// //                 a: "Yes. Files are processed securely and permanently deleted after splitting.",
// //               },
// //               {
// //                 q: "Can I split a PDF on my phone?",
// //                 a: "Yes. PDFLinx works on Android and iOS mobile devices, tablets, and all desktop browsers.",
// //               },
// //               {
// //                 q: "How do I combine split pages back into one PDF?",
// //                 a: "After splitting, use the Merge PDF tool — upload the individual page PDFs and download one merged PDF file.",
// //               },
// //             ],
// //             relatedTitle: "More PDF Tools",
// //             seoSections: [
...
// //                 text: "Separate large PDF documents into smaller individual page files without losing quality.",
// //               },
// //               {
// //                 title: "Keep Original PDF Quality",
// //                 text: "All split PDF pages preserve original text, images, and formatting.",
// //               },
// //               {
// //                 title: "Extract Individual PDF Pages",
// //                 text: "Convert every page of your PDF into separate standalone PDF files for easier sharing and organization.",
// //               },
// //               {
